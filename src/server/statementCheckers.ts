...
function parenting(parents: { [key: string]: Checker }): Checker {
	return (statement: Statement, parent?: Statement) => {
		...
		if (!Object.keys(parents).includes(parentStatement)) {
			return `Statement ${statement.statement} must be inside ${Object.keys(parents).join(", ")}, not ${parentStatement}`;
		} else {
...
function argumentOfType(argumentIndex: number, type: "quote" | "text" | "block" | "bracket"): Checker {
	return (statement: Statement, parent?: Statement) => {
		if (statement.args[argumentIndex].type != type) {
			return `Argument ${argumentIndex} of ${statement.statement} must be a ${formatType(type)}, not a ${formatType(statement.args[argumentIndex].type)}`;
		}
	}
}
...
export const checkers = new Map<string, Checker>()
	...
	.set("info", parenting({
		event: combine([
			hasAmtOfArgs(1),
			argumentOfType(0, "quote"),
		])
	}))
	.set("description", parenting({
		schedule: combine([
			hasAmtOfArgs(1),
			argumentOfType(0, "quote"),
		])
